<x-app-layout>
    <div class="max-w-3xl mx-auto px-6 py-8">
        ...
        <div class="mb-8">
            <h1 class="font-serif text-3xl text-[#f5f5f0]">{{ isset($room) ? 'Edit' : 'Add' }} Room</h1>
            <p class="text-[#8a8a8a] text-sm mt-1">{{ isset($room) ? 'Update room details' : 'Fill in room information' }}</p>
        </div>

        <form method="POST" action="{{ isset($room) ? route('admin.rooms.update', $room) : route('admin.rooms.store') }}" enctype="multipart/form-data" class="luxury-form-card luxury-form-inner space-y-6">
            @csrf
            @if(isset($room))
                @method('PUT')
            @endif
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="bed_type" id="bed_type" value="{{ old('bed_type', '') }}"
                        class="w-full px-4 py-2.5 bg-[#0f0f0f] border border-[#2a2a2a] rounded-lg text-[#f5f5f0] focus:outline-none focus:border-[#c9a77c] focus:ring-1 focus:ring-[#c9a77c] transition-all">
                ...
                <div>
                    ...
                    @error('rooms_available') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="number" name="capacity" id="capacity" value="{{ old('capacity', $room->capacity ?? 2) }}" required
                        class="w-full px-4 py-2.5 bg-[#0f0f0f] border border-[#2a2a2a] rounded-lg text-[#f5f5f0] focus:outline-none focus:border-[#c9a77c] focus:ring-1 focus:ring-[#c9a77c] transition-all">
